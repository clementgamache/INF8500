...
:m_timer_callbacks(timer_callbacks), m_count(count), m_timer_frequency(timer_frequency) {
	m_access_delay = std::abs(m_timer_callbacks.value()-m_timer_callbacks.value());
    reset();
}
